...
public class ConfigListWidget extends AbstractWidget {
    ...
    protected void extractWidgetRenderState(
        ...
        int fullLeft = fullContentLeft();
        int fullRight = fullLeft + this.fullContentWidth();
        ...
        for (ConfigListEntry entry : this.activeEntries()) {
            ...
            graphics.enableScissor(entry.fullWidth() ? fullLeft : contentLeft, top,
                    entry.fullWidth() ? fullRight : contentRight, bottom);
    ...
    private void ensureLayout() {
        ...
        this.scrollbarVisible = false;
        int baseContentWidth = Math.max(0, fullContentWidth() - ELEMENT_SIDE_PADDING * 2);
        ...
        if (this.contentHeight > viewportHeight) {
            ...
            this.contentWidth = Math.max(0, fullContentWidth() - ELEMENT_SIDE_PADDING * 2);
            this.contentHeight = this.layoutEntries(this.contentWidth);
        }
    ...
    private int layoutEntries(int contentWidth) {
        ...
        for (int i = 0; i < entries.size(); i++) {
            ...
            int x = this.getX() + elementPadding(entry);
            int width = entry.fullWidth() ? fullContentWidth() : contentWidth;

            entry.updateEntryLayout(x, y, width);
            y += entry.getHeight();
    ...
    private int contentLeft() {
        return this.getX() + ELEMENT_SIDE_PADDING;
    }

    private int fullContentLeft() {
        return this.getX();
    }
    ...
    private int fullContentWidth() {
        ...
        if (this.isScrollbarVisible()) {
            width -= SCROLLBAR_WIDTH + SCROLLBAR_GAP;
        }
