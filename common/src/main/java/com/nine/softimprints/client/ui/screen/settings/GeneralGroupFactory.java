...
import static com.nine.softimprints.client.ui.screen.settings.SettingsControls.booleanButton;
...
public class GeneralGroupFactory implements SettingsGroupFactory {
    ...
    private static void addModelContact(GroupBuildContext context, GroupBuilder builder) {
        ...
        builder.rowWidgets(
                offscreenPolicyControl(context),
                fallbackPolicyControl(context)
        );
        builder.rowWidgets(
                applyTooltip(
                        booleanButton(context, "config.softimprints.group.general.accumulate_snapshots", SIConfig.General.IMPRINT_ACCUMULATE_SNAPSHOTS),
                        Component.translatable("config.softimprints.group.general.accumulate_snapshots.tooltip")
                )
        );
        builder.rowWidgets(
                applyTooltip(
    ...
    private static Button offscreenPolicyControl(GroupBuildContext context) {
        var draft = context.editorContext().config().draft(SIConfig.General.MODEL_CONTACT_OFFSCREEN_POLICY);
        ModelContactOffscreenPolicy current = draft.getDraft();
        return Button.builder(offscreenPolicyText(current), button -> {
                    ModelContactOffscreenPolicy next = nextOffscreenPolicy(draft.getDraft());
                    draft.setDraft(next);
                    button.setMessage(offscreenPolicyText(next));
                    button.setTooltip(Tooltip.create(offscreenPolicyTooltip(next)));
                })
                .bounds(0, 0, 1, SettingsControls.DEFAULT_HEIGHT)
                .tooltip(Tooltip.create(offscreenPolicyTooltip(current)))
                .build();
    }

    private static Component offscreenPolicyText(ModelContactOffscreenPolicy policy) {
        return Component.translatable(
                "config.softimprints.group.general.offscreen_policy",
                Component.translatable("config.softimprints.group.general.offscreen_policy." + policy.id())
        );
    }

    private static Component offscreenPolicyTooltip(ModelContactOffscreenPolicy policy) {
        return Component.translatable("config.softimprints.group.general.offscreen_policy." + policy.id() + ".tooltip");
    }

    private static ModelContactOffscreenPolicy nextOffscreenPolicy(ModelContactOffscreenPolicy policy) {
        return switch (policy) {
            case DISABLED -> ModelContactOffscreenPolicy.ON_DEMAND;
            case ON_DEMAND -> ModelContactOffscreenPolicy.DISABLED;
        };
    }
    ...
    private static ModelContactFallbackPolicy nextFallbackPolicy(ModelContactFallbackPolicy policy) {
        return switch (policy) {
            ...
            case BOUNDING_BOX -> ModelContactFallbackPolicy.SKIP;
        };
    }
