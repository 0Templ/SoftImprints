...
import static com.nine.softimprints.client.ui.screen.settings.SettingsControls.intSlider;
...
public class StorageGroupFactory implements SettingsGroupFactory {
    ...
    public ListGroup build(GroupBuildContext context) {
        ...
        var offScreenCaptureSlider = maxAsUnlimitedSlider(
                context,
                "config.softimprints.group.storage.offscreen_captures_per_frame",
                SIConfig.Performance.MAX_OFFSCREEN_CAPTURES_PER_FRAME
        );
        applyOffscreenCaptureTooltip(offScreenCaptureSlider);
        builder.widget(offScreenCaptureSlider);
...
//                .label(Component.literal("This group is cached and does not rebuild on profile switch."))
    ...
    private static void applyOffscreenCaptureTooltip(ExtendedSlider slider) {
        slider.setTooltipFormatter(value -> {
            int intValue = Math.toIntExact(Math.round(value));
            return Component.translatable(intValue == 0
                    ? "config.softimprints.group.storage.offscreen_captures_per_frame.tooltip.disabled"
                    : "config.softimprints.group.storage.offscreen_captures_per_frame.tooltip");
        });
    }
