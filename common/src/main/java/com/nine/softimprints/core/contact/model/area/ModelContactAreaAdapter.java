package com.nine.softimprints.core.contact.model.area;
...
public final class ModelContactAreaAdapter {
    ...
    public static CompositeContactShape adapt(
        ...
        List<ModelContactSnapshotBox> contactBoxes = collectContactBoxes(
                snapshot.boxes(),
                localBandMinY,
                localBandMaxY,
                yawDeltaRadians
    ...
    private static List<ModelContactSnapshotBox> collectContactBoxes(
            List<ModelContactSnapshotBox> source,
            double bandMinY,
            double bandMaxY,
            double yawDeltaRadians
    ) {
        List<ModelContactSnapshotBox> result = new ArrayList<>(Math.min(24, source.size()));
        boolean rotate = Math.abs(yawDeltaRadians) > 1.0E-7D;
        for (ModelContactSnapshotBox box : source) {
            if (box.maxY() < bandMinY || box.minY() > bandMaxY) {
                continue;
            }
            result.add(rotate ? box.rotateAroundY(yawDeltaRadians) : box);
        }
        return result;
    }
