...
public class SIUpdateChecker implements UpdateChecker {
	...
	public SIUpdateInfo checkForUpdates() {
		...
		var tempVersion = FabricLoader.getInstance().getModContainer(SICommon.MODID)
		...
		var ret = update == null ? SIUpdateInfo.NONE :
				new SIUpdateInfo(true, Component.literal(update.version()), update.url(), update.updateChannel());
		return update == null ? SIUpdateInfo.NONE :
				new SIUpdateInfo(true, Component.literal(update.version()), update.url(), update.updateChannel());
	}
	...
	public static JsonObject tryGetUrlInfo() {
		try {
			...
			SICommon.LOGGER.debug("Couldn't fetch update info{}", e.getMessage());
		}
		return null;
	}
